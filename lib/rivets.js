...
(function() {
  ...
  Rivets.binders = {
    ...
    "options": function(el, value) {
      var config, match, mustache, o, option, options, outerHTML, r, s, template;
      if (!value) {
        return;
      }
      mustache = function(template) {
        ...
        r = /{{ *(\w+) *}}/g;
        return function(ob) {
          return template.replace(r, function(m, group) {
            var val;
            val = ob[group];
            if (val) {
              return val;
      ...
      } else if (config.value && config.label) {
        template = function(x) {
          return "<option value=\"" + x[config.value] + "\"> " + x[config.label] + " </option>";
        };
      } else {
        template = function(x) {
      ...
      options = (function() {
        ...
        for (_i = 0, _len = value.length; _i < _len; _i++) {
          o = value[_i];
          _results.push(template(o));
        }
        return _results;
      })();
      config.caption && options.unshift("<option value=\"\"> " + config.caption + " </option>");
      return el.innerHTML = options.join(" ");
    }
  };
